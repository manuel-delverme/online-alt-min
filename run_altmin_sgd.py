...
if __name__ == "__main__":
    ...
    print("Ignoring", sys.argv)
    if "^random_seed" in sys.argv:
        idx = sys.argv.index("^random_seed")
        del sys.argv[idx]
        print("Ignoring", sys.argv)

    args = parser.parse_args()
    experiment_buddy.register_defaults(vars(args))
    tb = experiment_buddy.deploy("mila", sweep_yaml="sweep_hyper.yaml", proc_num=10)
